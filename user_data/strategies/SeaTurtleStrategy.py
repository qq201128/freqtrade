...
class SeaTurtleStrategy(IStrategy):
    """
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        ##唐安琪通道
        dataframe["max_hight_20"] = dataframe["high"].rolling(window=20).max()
        dataframe["max_hight_10"] = dataframe["high"].rolling(window=10).max()
        if 'max_hight_20' not in dataframe.columns:
            raise ValueError("Column 'max_hight_20' not found in DataFrame.")
        dataframe["max_low_20"] = dataframe["low"].rolling(window=20).max()
        dataframe["max_low_10"] = dataframe["low"].rolling(window=10).max()
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        entry_long_1 = (
                #当价格高于前20个周期的最高价就买多
                (dataframe["tema"] >= dataframe["max_low_10"])
                &
                (dataframe["volume"] > 0)
        )
